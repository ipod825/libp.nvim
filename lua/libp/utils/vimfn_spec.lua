...
describe("buf_get_line", function()
    ...
    it("Defaults buffer to 0 ", function()
        assert.are.same("a", vimfn.buf_get_line({ row = 0 }))
        assert.are.same("b", vimfn.buf_get_line({ row = 1 }))
        assert.are.same("c", vimfn.buf_get_line({ row = 2 }))
        assert.has.errors(function()
            vimfn.buf_get_line({ row = 3 })
        end)
    end)
