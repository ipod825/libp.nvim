...
describe("tick", function()
    describe("total is known", function()
        it("Defaults to no desc", function()
            ...
            p:tick(1)
            assert.is_true(vim.startswith(vimfn.buf_get_line({buffer=p.buffer.id, row=0}), "█"))
        end)
        it("Respects input desc", function()
            ...
            p:tick(1)
            assert.is_true(vim.startswith(vimfn.buf_get_line({buffer=p.buffer.id, row=0}), "Desc:█"))
        end)
